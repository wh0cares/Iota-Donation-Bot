...
functions.sendIota = function (discord_id, receiving_discord_id, amount) {
    return new Promise(resolve => {
        bot.iota.api.getNewAddress(config.iota.seed, { checksum: true, total: 2 }, function (error, address) {
            if (address) {
                bot.iota.api.sendTransfer(config.iota.seed, 4, 14, [{ 'address': address[0], 'value': 0 }, { 'address': address[1], 'value': 0 }], function (error, bundle) {
                    if (error) {
                        resolve(["error"]);
                    } else {
                        bot.iota.api.sendTransfer(config.iota.seed, 4, 14, [{ 'address': address[0], 'value': amount }], { address: bot.iota.utils.noChecksum(address[1]) }, function (error, bundle) {
                            if (error) {
                                resolve(["error"]);
                            } else {
                                db.none('UPDATE users SET balance = balance - ${balance} WHERE discord_id = ${discord_id};UPDATE users SET balance = balance + ${balance} WHERE discord_id = ${receiving_discord_id}', {
                                    balance: amount,
                                    discord_id: discord_id,
                                    receiving_discord_id: receiving_discord_id
                                }).then(success => {
                                    resolve(["success", bundle[0].hash]);
                                }).catch(error => {
                                    resolve(["error"]);
                                });
                            }
